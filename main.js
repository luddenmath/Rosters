...
// Preferred: use links already present on the TAC home page
classes = [...document.querySelectorAll('a[href*="/TAC/ClassRoster?SectionKey="]')]
    .map(a => {
        const url = new URL(a.href);

        return {
            Description: a.textContent.trim(),
            Periods: (url.searchParams.get("Periods") || "").trim(),
            Url: a.href,
...
// Fallback: original Class Chooser method
if (!classes.length && $("#toolbarButtonClassChooser").length) {
    ...
    classes = $("#classChooserGrid").jqGrid("getRowData");
